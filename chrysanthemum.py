...
class MainWindow(QtGui.QMainWindow):
    ...
    def dropEvent(self, event):
        self.file_list = [str(url.toLocalFile()) for url in event.mimeData().urls()]
        self.open_file_list()

    def open_file_list(self):
        map(lambda x:fill_model(walk_code_object(load_code_object(x)), self.code_object_view.model()), self.file_list)
    ...
    @QtCore.pyqtSlot()
    def open(self):
        self.file_list = map(str, QtGui.QFileDialog.getOpenFileNames(self, filter='Python files (*.py *.pyw *.pyc *.pyo);;All files (*.*)'))
        self.open_file_list()
